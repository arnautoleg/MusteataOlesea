...
# Null hypothesis value (usually 0.5 for a two-sided test)
p0 <- 0.5

# Create the binary sample
sample <- c(rep(1, x), rep(0, n - x))

# Shuffle the sample to randomize the order

sample <- sample(binary_sample)

stdev <- sqrt(p0*(1-p0))

# Perform the z-test
z_test_result <- BSDA::z.test(sample, mu = p0, alternative = "two.sided", sigma.x=stdev)
...
observed_successes <- 70

# Perform the binomial test
binom_test_result <- binom.test(observed_successes, n, p = expected_proportion, alternative = "two.sided")
...
# Функция для проведения z-теста для двух долей
...
# x <- matrix(c(12, 7, 7, 0), ncol = 2)
# chisq.test(x)$p.value           
# chisq.test(x, simulate.p.value = TRUE, B = 10000)$p.value


x <- matrix(c(9, 1, 1, 9), ncol = 2)
chisq.test(x)   

# Выполните тест хи-квадрат
chi_square_test <- chisq.test(x, simulate.p.value = TRUE, B = 2000)
...
my_matrix <- matrix(c(9, 1, 1, 9), nrow = 2, ncol = 2, dimnames = list(c("Ринит +", "Ринит -"), c("Ринит +", "Ринит -")))
