...
## Асимптотический ДИ (Wald) и Точный ДИ (Clopper-Pearson)
...
n <- 10 # Размер выборки
x <- 1  # Количество успешных наблюдений
...
# Выведите результаты с красивым форматированием

cat("\nМетод Уолда (Wald):\n")
cat("Нижний предел, Верхний предел :", wald_ci$lower, wald_ci$upper, "Ширина:", wald_ci$upper - wald_ci$lower,"\n")

cat("Метод Клоппера-Пирсона (Clopper-Pearson):\n")
cat("Нижний предел, Верхний предел:", pearson_ci$lower, pearson_ci$upper, "Ширина:", pearson_ci$upper - pearson_ci$lower,"\n")
...
# Вывод результатов
cat("Метод Уолда (Wald) успешно охватил истинное значение в ", wald_success_rate, "% симуляций.\n")
cat("Метод Клоппера-Пирсона (Clopper-Pearson) успешно охватил истинное значение в ", pearson_success_rate, "% симуляций.\n")
...
# Установите и загрузите пакет ExactCIdiff, если это еще не было сделано
...
cat("Asymptotic CI (95%):", asym_lower, asym_upper, "Ширина:", asym_upper - asym_lower, "\n")
cat("Exact CI (95%)     :", exact_lower, exact_upper, "Ширина:", exact_upper - exact_lower, "\n")
...
cat("Exact Paired CI (95%):", round(result$ExactCI[1], 2), round(result$ExactCI[2], 2), ", Ширина:", round(result$ExactCI[2] - result$ExactCI[1], 2), "\n")
...
for (i in 1:n_simulations) {
  ...
  if (true >= asym_ci$conf.int[1] && true <= asym_ci$conf.int[2]) {
    asymptotic_hits <- asymptotic_hits + 1
  }
  if (true >= exact_ci$ExactCI[1] && true <= exact_ci$ExactCI[2]) {
    exact_hits <- exact_hits + 1
  }
}

# Оцениваем, сколько раз ДИ поймали истинное значение
cat("Asymptotic CI Hit Rate:", asymptotic_hits / n_simulations, "\n")
cat("Exact CI Hit Rate:", exact_hits / n_simulations, "\n")
...
# Доверительный интервал для OR (отношение шансов) https://search.r-project.org/CRAN/refmans/epitools/html/oddsratio.html
...
a <- 9  # Number of observations in Group 1 with Outcome 1
b <- 1  # Number of observations in Group 2 with Outcome 1
c <- 1  # Number of observations in Group 1 with Outcome 2
d <- 9  # Number of observations in Group 2 with Outcome 2
